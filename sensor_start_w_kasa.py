...
class Kasa(object):
	...
	def get_auth_token(self, uuid, username, password):
		auth_obj = {
			"method": "login", 
			"params": {
				"appType": "Kasa_Andriod",
	...
	def set_dev_state(self, token, sel_device_id, sel_device_state):
		set_obj = {
	...
	def set_dev_state_emeter(self, token, sel_device_id):
		set_obj = {
			"method": "passthrough", "params": {
				"deviceId": sel_device_id,
...
def saveResults():
	DBSETUP.ganacheLogger(float(os.environ.get('TEMP_MODIFIER'))*float(temperature), "Temperature", "C", "MAC_T", "unit_descrip", "SHTC3", "Sensirion")	
	DBSETUP.ganacheLogger(float(os.environ.get('HUM_MODIFIER'))*float(relative_humidity), "Humidity", "%", "MAC_H", "unit_descrip", "SHTC3", "Sensirion")
## T6713
	DBSETUP.ganacheLogger(float(os.environ.get('CO2_MODIFIER'))*float(obj_6713.gasPPM()), "CO2 Concentration", "PPM", "MAC_CO2", "unit_descrip", "T6713", "Amphenol Advanced Sensors")
